...
fun GameScreen(viewModel: SimonGameVM) {
  ...
  val soundList = viewModel.getSoundList()
  ...
  Column(
    ...
    Row {
      SquareButton(soundList[0], isSoundButtonBlocked, viewModel.isButtonBacklightEnabled) {
        viewModel.addPlayerSequence(soundList[0])
      }
      SquareButton(soundList[1], isSoundButtonBlocked, viewModel.isButtonBacklightEnabled) {
        viewModel.addPlayerSequence(soundList[1])
      }
    }
    Row {
      SquareButton(soundList[2], isSoundButtonBlocked, viewModel.isButtonBacklightEnabled) {
        viewModel.addPlayerSequence(soundList[2])
      }
      SquareButton(soundList[3], isSoundButtonBlocked, viewModel.isButtonBacklightEnabled) {
    ...
    if (viewModel.gameMode == GameMode.DEFAULTGAME) RectangleButton(playButtonText.value) {
      ...
      coroutineScope.launch {
        delay(viewModel.soundDelay.value * lvl.value.toLong())
